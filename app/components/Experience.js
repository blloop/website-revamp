...
export default function Experience({ experience, index }) {
  return (
    ...
    >
      <div className={`absolute p-2 left-12 top-[3.5rem] transform -translate-x-1/2 w-20 h-20 bg-olive-300 rounded-full z-10 ${index % 2 === 0 ? "md:-right-20 md:left-auto" : "md:left-0"}`}>
        {experience.image &&
        <Image
          src={urlForImage(experience.image).auto("format").size(256, 256).url()}
          width={256}
          height={256}
      ...
      <div className={`absolute w-0 h-0 top-20
        border-gray-600 
